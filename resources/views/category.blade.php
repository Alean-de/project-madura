            ...
            <div class="table-responsive">
                <table class="table align-middle table-hover mb-0">
                    ...
                    <tbody class="border-0">
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <span class="badge bg-light text-dark border px-3 py-1.5 rounded-pill fw-semibold fs-7">
                                    <i class="bi bi-box-seam text-secondary me-1"></i> {{ number_format($c->products_count, 0, ',', '.') }} Produk
                                </span>
                            </td>
                            <td class="text-center">
                                @if ($c->status)
                        ...
                        {{-- DELETE MODAL --}}
                        {{-- <div class="modal fade" id="deleteModal{{ $c->category_id }}" tabindex="-1" aria-hidden="true">
                                ...
                                <div class="modal-content border-0 shadow rounded-4">
                                    <div class="modal-body text-center p-4">
                                        ...
                                        <p class="text-muted small mb-4">
                                            Yakin ingin menghapus kategori <strong>{{ $c->category_name }}</strong>? Data produk terkait mungkin akan terpengaruh.
                                        </p>
                        ...
                        {{-- EDIT MODAL
                        <div class="modal fade" id="editModal{{ $c->category_id }}" tabindex="-1" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered">
                                <div class="modal-content border-0 shadow rounded-4">
                                    <div class="modal-header border-0 px-4 pt-4 pb-0">
                                        <h5 class="fw-bold text-dark d-flex align-items-center gap-2 mb-0">
                                            <i class="bi bi-pencil-style text-primary fs-4"></i> Ubah Data Kategori
                                        </h5>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                    </div>

                                    <form action="{{ route('kategori.update', $c->category_id) }}" method="POST">
                                        @csrf
                                        @method('PUT')
                                        <div class="modal-body p-4">
                                            <div class="mb-3">
                                                <label class="form-label fw-semibold text-secondary small mb-1">Nama Kategori</label>
                                                <input type="text" name="category_name" class="form-control py-2 rounded-3" value="{{ $c->category_name }}" required>
                                            </div>

                                            <div class="mb-3">
                                                <label class="form-label fw-semibold text-secondary small mb-1">Jumlah Produk (Penyesuaian Manual)</label>
                                                <input type="number" name="product_count" class="form-control py-2 rounded-3" value="{{ $c->product_count }}">
                                            </div>

                                            <div class="mb-3">
                                                <label class="form-label fw-semibold text-secondary small mb-1">Status Kategori</label>
                                                <select name="status" class="form-select py-2 rounded-3">
                                        ...
                                        <div class="modal-footer border-0 px-4 pb-4 pt-0">
                                            ...
                                            <button type="submit" class="btn text-white px-4 py-2 border-0 rounded-3 shadow-sm" style="background-color: #17354D;">Simpan</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
